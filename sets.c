...
int set(t_set_call *param) {
    t_utils misc_utils;
    t_mlx *mlx = param->screen;
    t_complex z;
    t_complex c;

	if (param->mandelbrot == FALSE) c = *param->c;
    ...
    while (misc_utils.y < WIN_HEIGHT) {
        ...
        while (misc_utils.x < WIN_WIDTH) {
            ...
            my_mlx_pixel_put(&(mlx->img), misc_utils.x, misc_utils.y, get_color(misc_utils.iter, param->color_factor));
            misc_utils.x++;
        }
        misc_utils.y++;
    }
